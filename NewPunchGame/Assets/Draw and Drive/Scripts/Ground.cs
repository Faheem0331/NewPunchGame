    ...
    public class Ground : MonoBehaviour
    {
        ...
        private void changeStateInAsyncMode()
        {
            foreach (CarObject node in carObjects)
            {
                ...
                {
                    PlayerAnimationController.Instance.PlayerAnimationPlay(0);
                    StartCoroutine(StartMovingPunch(node));
                    // node.playerState = State.Movement;
                }
            }
        }

        public IEnumerator StartMovingPunch(CarObject node)
        {
            yield return new WaitForSeconds(0.25f);
            PlayerAnimationController.Instance.CurrentPunchOff(0,false);
