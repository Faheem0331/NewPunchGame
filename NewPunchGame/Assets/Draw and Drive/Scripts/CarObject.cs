...
namespace Canvas
{
    ...
    #region enums
    public enum State { Null, Start, Draw, Ready, Movement,ComeBack, Finished }
    #endregion
    public class CarObject : MonoBehaviour
    {
       ...
       // public State currentState = State.Start;
        ...
        private int currentBackNodeIndex;
        ...
        public State _playerState = State.Null;
            ...
            set
            {
                switch (value)
                {
                    ...
                    case State.Movement:
                        ...
                        break;
                    case State.ComeBack:
                        if (_playerState != State.ComeBack)
                        {
                            runBackEvent();
                        }
                        break;
        ...
        private void FixedUpdate()
        {
            if (playerState == State.Movement)
            {
                ...
                PlayerAnimationController.Instance.generateArm.play = true;
                Moving();
                move(targetPosition, targetHitDistance);
            }

            if (playerState == State.ComeBack)
            {
                MoveBack();
                move(targetPosition, targetHitDistance);
            }
        ...
        public void Clear()
        {
            ...
            nodes.Clear();
            lines.Clear();
            gameObject.transform.GetChild(0).transform.localScale = new Vector3(85, 85, 85);
            gameObject.transform.GetChild(0).gameObject.SetActive(false);
            PlayerAnimationController.Instance.nowRemove = false;
            PlayerAnimationController.Instance.movingGlove.transform.DORotate(new Vector3(0,0,0), 0);
            PlayerAnimationController.Instance.generateArm.firstTime = false;
            PlayerAnimationController.Instance.CurrentPunchOff(0,true);
        ...
        private void Moving()
        {
            ...
            if (currentIndex == 0)
            {
                ...
                gameObject.transform.GetChild(0).transform.localScale = new Vector3(85, 85, 85);
                setPosition(new Vector3(nodes[0].gameObject.transform.position.x, height + nodes[0].gameObject.transform.position.y, nodes[0].gameObject.transform.position.z));
            }
            if (currentIndex < nodes.Count)
            {
                if (targetPosition == Vector3.zero)
                {
                    ...
                    targetPosition = nodes[currentIndex].gameObject.transform.position;
                    currentIndex++;
                }
            }
            else
            {
                PlayerAnimationController.Instance.generateArm.play = false;
                PlayerAnimationController.Instance.generateArm.firstTime = false;
                if (PlayerAnimationController.Instance.isEnemyDie)
                {
                    gameObject.transform.GetChild(0).transform.gameObject.SetActive(false);
                    Clear();
                    PlayerAnimationController.Instance.AssignTargetPosition();
                    playerState = State.Start;
                    gameObject.transform.GetChild(0).transform.localScale = new Vector3(85, 85, 85);
                }
                else
                {
                    PlayerAnimationController.Instance.nowRemove = true;
                    gameObject.transform.GetChild(0).transform.localScale = new Vector3(85, -85, 85);
                    playerState = State.ComeBack;
                    speed = 150;
                }
            }
        }
        ...
        private void MoveBack()
        {
            if (currentIndex <= nodes.Count)
            {
                if (targetPosition == Vector3.zero)
                {
                    calculateReverseBendAngle(currentIndex);
                    targetPosition = nodes[currentIndex-1].gameObject.transform.position;
                    currentIndex--;
                }
        ...
        private void calculateReverseBendAngle(int index)
        {
            ...
            {
                Vector3 v1 = nodes[index-1].gameObject.transform.position - transform.position;
                Vector3 v2 = nodes[index-2].gameObject.transform.position - nodes[index-1].gameObject.transform.position;
                bendAngle = Vector3.Angle(v1, v2);
            }
        }
